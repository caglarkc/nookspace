...
// Get NookSpace data directory for persistent storage
// Use platform-appropriate paths:
// - macOS: ~/Library/Application Support/nookspace
// - Windows: %APPDATA%/nookspace
const OPEN_COWORK_DATA_DIR = PLATFORM === 'win32'
  ? path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'nookspace')
  : path.join(os.homedir(), 'Library', 'Application Support', 'nookspace');
...
async function executeCliclick(command: string): Promise<{ stdout: string; stderr: string }> {
  ...
  if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
    const hint =
      ...
      '- System Settings → Privacy & Security → Accessibility：允许 NookSpace\n' +
      '- 如果是终端运行：允许 Terminal/iTerm\n' +
      '- 授权后请重启 NookSpace 再重试\n';
  ...
  return result;
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Accessibility：允许 NookSpace\n' +
      '- System Settings → Privacy & Security → Automation：允许 NookSpace 控制 “System Events”\n';
...
async function takeScreenshot(
  ...
  await executeCommand(command);
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Screen Recording：允许 NookSpace\n' +
...
async function callVisionAPIWithTimeout(
  ...
  if (isOpenAICompatible) {
    ...
    if (isOpenRouter) {
      headers['HTTP-Referer'] = 'https://github.com/caglarkc/nookspace';
      headers['X-Title'] = 'NookSpace';
    }
